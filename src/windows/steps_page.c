...
//   // also need 40px icons!!!
  ...
  s_cat_bitmap = gbitmap_create_with_resource(RESOURCE_ID_CATSTEPS_IMAGE);
  s_cat_bitmap_layer = bitmap_layer_create(GRect(bounds.size.w/2-25,text_layer_get_content_size(s_steps_text_layer).h+text_layer_get_content_size(s_youveTaken_text_layer).h+5, icon_size + 20, icon_size + 20));
